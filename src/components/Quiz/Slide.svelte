...
<section>
	...
	z"
		...
		<div class="bottom">
			<div class="btns">
				<button on:click={() => selectAnswer("yes")}>
					<span>Yes</span>
				</button>
				<button on:click={() => selectAnswer("no")}>
					<span>No</span>
				</button>
				<button on:click={() => selectAnswer("no answer")}>
					<span>Skip</span>
				</button>
			</div>
		</div>
...
<style>
	* {
	...
	.bg {
		...
		opacity: 0.2;
		z-index: 1;
		background-image: url("https://electeddc.org/wp-content/uploads/2022/08/elected-students-first-uai-900x506.jpg");
	}
	.bottom {
		background: #c7871e;
		padding: 0 20px;
